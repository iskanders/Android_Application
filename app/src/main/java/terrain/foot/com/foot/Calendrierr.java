...
listview = (ListView) findViewById(R.id.timelist);
    ...
    public  void   adapterlist(){
        Query query=myRef.child(id).child("reservation").orderByChild("date").equalTo(date);
...
ValueEventListener valuee1=new ValueEventListener() {
    @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN)
    @Override
    ...
    public void onCancelled(DatabaseError databaseError) {
        ...
        myRef.child(id).child("tempBloquer").child(date).addListenerForSingleValueEvent(valuee1);
                ...
                if(list.get(position).getEtat()==0)
                {
                    ...
                    builder.setMessage("voulez vous Blocker ce temp").setPositiveButton("oui", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            im[0].setBackgroundResource(R.color.red);
