...
pub.clear();
    ...
    public void remplipub(int nbr, final View view, final LayoutInflater inflater){
                ...
                if(dataSnapshot.exists()) {
                    ...
                    for (final DataSnapshot data : dataSnapshot.getChildren()) {
                        if(data.hasChild("text") && data.hasChild("img")) {
                            pub.add(new Feedback_item(data.child("text").getValue().toString(), data.child("img").getValue().toString(),data.child("name").getValue().toString()+" "+data.child("lastname").getValue().toString(),data.getKey(),data.child("photoProfile").getValue().toString()));
                        }else if(!data.hasChild("text")){
                            pub.add(new Feedback_item("", data.child("img").getValue().toString(), data.child("name").getValue().toString()+" "+data.child("lastname").getValue().toString(),data.getKey(),data.child("photoProfile").getValue().toString()));
                        }else if(!data.hasChild("img")){
                            pub.add(new Feedback_item(data.child("text").getValue().toString(),"",  data.child("name").getValue().toString()+" "+data.child("lastname").getValue().toString(),data.getKey(),data.child("photoProfile").getValue().toString()));
                        }
        ...
        query.addValueEventListener(valuea);
    }
    @SuppressLint("ValidFragment")
